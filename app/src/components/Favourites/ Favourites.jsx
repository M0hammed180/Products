...
export default function Favourites() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((response) => {
        if (response.data.myFavourites) setFavourites(response.data.myFavourites.products || []);
      })
      ...
      <section className="mx-auto w-full max-w-6xl">
        ...
        {products.length === 0 ? (
          <div className="rounded-2xl border border-zinc-700 bg-zinc-800 p-8 text-center shadow-xl">
            ...
            <p className="mt-2 text-sm text-zinc-400">احفظ المنتجات التي تعجبك وستظهر هنا.</p>
          ...
          <>
            <div className="hidden overflow-hidden rounded-2xl border border-zinc-700 shadow-xl md:block">
              <table className="w-full min-w-[680px] text-left text-sm text-zinc-300">
                <thead className="bg-zinc-900 text-xs uppercase text-zinc-400">
                  <tr>
                    <th scope="col" className="px-5 py-4">المنتج</th>
                    <th scope="col" className="px-5 py-4">السعر</th>
                    <th scope="col" className="px-5 py-4 text-left">الإجراءات</th>
                  </tr>
                </thead>
                <tbody>
                  {products.map((product) => (
                    <tr key={product._id} className="border-t border-zinc-700 bg-zinc-800 transition hover:bg-zinc-700">
                      <td className="px-5 py-4">
                        <div className="flex min-w-0 items-center gap-4">
                          <img src={product.images?.[0] || ""} className="h-20 w-16 shrink-0 rounded-lg bg-zinc-900 object-cover" alt={product.name} />
                          <span className="truncate font-semibold text-white">{product.name}</span>
                        </div>
                      </td>
                      <td className="whitespace-nowrap px-5 py-4 font-semibold text-white">${product.price}</td>
                      <td className="px-5 py-4">
                        <div className="flex justify-end gap-2">
                          <WhatsAppButton message={productWhatsAppMessage({ name: product.name, price: product.price, image: product.images?.[0] })} className="whitespace-nowrap">
                            التحدث بخصوص المنتج
                          </WhatsAppButton>
            ...
            <div className="space-y-3 md:hidden">
              {products.map((product) => (
                <article key={product._id} className="rounded-2xl border border-zinc-700 bg-zinc-800 p-4 shadow-lg">
                  <div className="flex min-w-0 gap-3">
                    <img src={product.images?.[0] || ""} className="h-24 w-20 shrink-0 rounded-xl bg-zinc-900 object-cover" alt={product.name} />
                    <div className="min-w-0 flex-1">
                      <h2 className="truncate font-semibold text-white">{product.name}</h2>
                      <p className="mt-2 text-lg font-bold text-white">${product.price}</p>
                    </div>
                  </div>
                  <div className="mt-4 grid grid-cols-1 gap-2 border-t border-zinc-700 pt-3 sm:grid-cols-2">
                    <AddToCartButton product={product} />
                    <RemoveButton product={product} />
                  </div>
                  <WhatsAppButton message={productWhatsAppMessage({ name: product.name, price: product.price, image: product.images?.[0] })} className="mt-2 w-full">
                    التحدث بخصوص المنتج
                  </WhatsAppButton>
                </article>
              ))}
